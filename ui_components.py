...
def _merge_skills_display(
    base_skills: List[str],
    extracted_skills: Optional[Dict[str, Any]],
) -> List[str]:
    skills = [s.strip() for s in (base_skills or []) if s and str(s).strip()]
    seen = {s.lower() for s in skills}
    if not extracted_skills or not isinstance(extracted_skills, dict):
        return skills
    for key in ("technical_skills", "soft_skills", "tools_software"):
        for s in extracted_skills.get(key) or []:
            if not s:
                continue
            t = str(s).strip()
...
def _render_bullet_list(items: List[str], empty_msg: str) -> None:
    cleaned = [x.strip() for x in (items or []) if x and str(x).strip()]
    if not cleaned:
...
def render_parsed_resume_sections(
    profile: Dict[str, Any],
    extracted_skills: Optional[Dict[str, Any]] = None,
    expanded: bool = True,
) -> None:
    """
    Show parsed resume fields: name, contact, skills, education, experience, qualifications.
    """
    skills = _merge_skills_display(profile.get("skills") or [], extracted_skills)

    with st.expander("📑 Parsed resume — extracted fields", expanded=expanded):
        st.caption(
            "Heuristic extraction from your document. Verify details before sharing externally."
        )
        ...
        with top_l:
            ...
            if profile.get("portfolio"):
                st.write(f"**Portfolio:** {profile['portfolio']}")
        with top_r:
            st.markdown("#### 🛠️ Skills")
            if skills:
                st.markdown(
                    " ".join(f"`{s}`" for s in skills[:60])
                    + (" …" if len(skills) > 60 else "")
                )
            else:
                st.caption("No dedicated skills block detected; try a clearer Skills section.")
        ...
        st.markdown("#### 🏅 Qualifications & certifications")
        _render_bullet_list(
            profile.get("qualifications") or [],
            "No certifications or licenses section detected.",
        )