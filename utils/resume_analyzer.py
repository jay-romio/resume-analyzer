...
class ResumeAnalyzer:
    ...
    def _extract_candidate_name(self, text):
        """Pick the most likely name line from the resume header (not email/URL/section title)."""
        email_pattern = r'[\w\.-]+@[\w\.-]+\.\w+'
        lines = [ln.strip() for ln in text.split('\n') if ln.strip()]
        section_re = re.compile(
            r'^(experience|work history|employment|education|skills|summary|objective|profile|'
            r'projects|certifications?|qualifications?|awards|references|contact)\b',
            re.I,
        )
        for line in lines[:20]:
            ...
            if re.search(email_pattern, line):
                continue
            if re.search(r'linkedin\.com|github\.com|http[s]?://', line, re.I):
                continue
    ...
    def extract_personal_info(self, text):
        ...
        name = self._extract_candidate_name(text)
    ...
    def extract_education(self, text):
        ...
        education_keywords = [
            ...
            'school', 'institute', 'diploma', 'bachelor', 'master',
            'phd', 'b.tech', 'm.tech', 'b.e', 'm.e', 'b.sc', 'm.sc','bca', 'mca', 'b.com',
            'm.com', 'b.cs-it', 'imca', 'bba', 'mba', 'honors', 'scholarship'
        ]
        in_education_section = False
        current_entry = []
        qual_section_hdr = re.compile(
            r'^(certifications?|professional certifications|licenses?|credentials?)\s*$',
            re.I,
        )
        ...
        for line in lines:
            ...
            if in_education_section:
                if qual_section_hdr.match(line):
                    in_education_section = False
                    if current_entry:
    ...
    def extract_qualifications(self, text):
        """Extract certifications, licenses, and professional credentials (separate from degree education)."""
        qualifications = []
        lines = text.split('\n')
        qual_keywords = [
            'certification', 'certifications', 'certificate', 'certificates',
            'license', 'licenses', 'licenced', 'licensed', 'credential', 'credentials',
            'accreditation', 'professional license', 'professional certifications',
            'aws certified', 'azure certified', 'google certified', 'cisco certified',
            'comptia', 'pmp', 'scrum master', 'safe', 'itil', 'six sigma',
            'professional development', 'training & certifications',
        ]
        resume_section_words = self.document_types['resume']
        in_section = False
        current_entry = []

        for line in lines:
            line = line.strip()
            header_hit = any(kw.lower() in line.lower() for kw in qual_keywords)
            if header_hit:
                if not any(kw.lower() == line.lower() for kw in qual_keywords):
                    current_entry.append(line)
                in_section = True
                continue

            if in_section:
                if line and any(kw.lower() in line.lower() for kw in resume_section_words):
                    if not any(qk.lower() in line.lower() for qk in qual_keywords):
                        in_section = False
                        if current_entry:
                            qualifications.append(' '.join(current_entry))
                            current_entry = []
                        continue
    ...
    def analyze_resume(self, resume_data, job_requirements):
        ...
        try:
            ...
            qualifications_suggestions = []
            if not qualifications:
                qualifications_suggestions.append(
                    "Add certifications or professional licenses if relevant to your target role"
                )
            ...
            suggestions.extend(qualifications_suggestions)
            ...
            return {
                **personal_info,  # Include extracted personal info
                ...
                'qualifications_suggestions': qualifications_suggestions,
