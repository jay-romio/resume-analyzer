...
from ui_components import (
    ...
    render_suggestions_section, render_parsed_resume_sections
)
from utils.structured_resume_parser import parse_resume_text, profile_from_analysis
...
from utils.file_upload_validator import enhanced_file_uploader
from utils.enhanced_analyzer import EnhancedSkillExtractor, EnhancedATSScorer
from utils.interview_generator import InterviewQuestionGenerator
from utils.interview_coach import AIInterviewCoach, render_interview_coach_interface
from utils.voice_interview_ui import render_voice_mock_interview as render_voice_mock_interview_page
from utils.resume_interview_ui import render_resume_based_interview
from utils.learning_roadmap import SkillLearningRoadmapGenerator
import traceback
...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "INTERVIEW COACH": self.render_interview_coach,
            "VOICE MOCK INTERVIEW": self.render_voice_mock_interview,
            "RESUME-BASED INTERVIEW": self.render_resume_based_interview_page,
            "LEARNING ROADMAP": self.render_learning_roadmap,
        ...
        # Load external CSS (path relative to app location, not shell CWD)
        _css_path = _APP_ROOT / "style" / "style.css"
    ...
    "Specific Role", roles, key="standard_role")
            ...
            uploaded_file = enhanced_file_uploader(
                "Upload your resume",
                key="standard_file",
                help_text="Upload your resume for comprehensive ATS analysis and skill extraction"
            )
            ...
            if uploaded_file:
                ...
                with b2:
                    preview_parse = st.button("📑 Preview extracted fields",
                                        use_container_width=True,
                                        key="preview_parse_standard")

                if preview_parse:
                    with st.spinner("Extracting text and parsing sections..."):
                        pt = ""
                        try:
                            if uploaded_file.type == "application/pdf":
                                pt = self.analyzer.extract_text_from_pdf(uploaded_file)
                            elif uploaded_file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                                pt = self.analyzer.extract_text_from_docx(uploaded_file)
                            else:
                                pt = uploaded_file.getvalue().decode(errors="replace")
                        except Exception as ex:
                            st.error(f"Could not read file: {ex}")
                        if pt and pt.strip():
                            preview_profile = parse_resume_text(pt, self.analyzer)
                            render_parsed_resume_sections(preview_profile, expanded=True)
                        elif not pt:
                ...
                if analyze_standard:
                    with st.spinner("Analyzing your document..."):
                        ...
                        # Analyze the document with enhanced analyzer
                        try:
                            # Use enhanced ATS scorer
                            enhanced_analysis = self.enhanced_analyzer.calculate_ats_score(text, selected_role)
                            
                            # Extract skills with enhanced extractor
                            extracted_skills = self.skill_extractor.extract_skills_advanced(text)
                            
                            # Combine analyses
                            analysis = {
                                **self.analyzer.analyze_resume({'raw_text': text}, role_info),
                                'enhanced_ats_score': enhanced_analysis,
                            ...
                            st.error(f"Enhanced analysis failed: {str(analysis_error)}")
    ...
    f"⚠️ This appears to be a {
        analysis['document_type']} document, not a resume!")
                        ...
                        prof = profile_from_analysis(analysis)
                        render_parsed_resume_sections(
                            prof,
                        ...
                        try:
                            ...
                            with st.spinner("🧠 AI is analyzing your resume..."):
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    try:
                                        ai_parsed = parse_resume_text(
                                            resume_text, self.analyzer)
                                        render_parsed_resume_sections(
                                            ai_parsed, expanded=True)
                                    except Exception:
                                        pass
    ...
    def render_home(self):
        ...
        # Features Heading with styling
        st.markdown("""
        <div style="text-align: center; margin: 40px 0;">
            <h2 style="font-size: 2.5rem; color: #667eea; margin-bottom: 10px;">
            ...
            <p style="color: #666; font-size: 1.2rem;">Powerful tools to transform your career journey</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Feature Cards Grid
        st.markdown('<div class="feature-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px; margin: 20px 0;">', unsafe_allow_html=True)
        
        # Feature 1: Resume Analysis
        st.markdown("""
        <div class="feature-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; padding: 30px; color: white; box-shadow: 0 10px 30px rgba(102, 126, 234, 0.3); transition: transform 0.3s ease, box-shadow 0.3s ease;">
            <div style="font-size: 3rem; margin-bottom: 20px; text-align: center;"> </div>
            <h3 style="font-size: 1.5rem; margin-bottom: 15px; text-align: center;">AI Resume Analysis</h3>
            <p style="line-height: 1.6; margin-bottom: 20px;">Get instant AI-powered feedback on your resume with advanced ATS scoring, skill extraction, and personalized recommendations to help you stand out from the competition.</p>
            <ul style="list-style: none; padding: 0;">
                <li style="margin-bottom: 8px;"> ATS Score Analysis</li>
                <li style="margin-bottom: 8px;"> Skill Gap Detection</li>
                <li style="margin-bottom: 8px;"> Keyword Optimization</li>
                <li style="margin-bottom: 8px;"> Format Recommendations</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        
        # Feature 2: Enhanced AI Interview Coach
        st.markdown("""
        <div class="feature-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border-radius: 20px; padding: 30px; color: white; box-shadow: 0 10px 30px rgba(245, 87, 108, 0.3); transition: transform 0.3s ease, box-shadow 0.3s ease;">
            <div style="font-size: 3rem; margin-bottom: 20px; text-align: center;"> </div>
            <h3 style="font-size: 1.5rem; margin-bottom: 15px; text-align: center;">AI Interview Coach</h3>
            <p style="line-height: 1.6; margin-bottom: 20px;">Experience ChatGPT-like conversational interviews with live analysis! Our AI coach provides real-time scoring, dynamic questions, and personalized feedback as if you're talking to a real interviewer.</p>
            <ul style="list-style: none; padding: 0;">
                <li style="margin-bottom: 8px;"> Natural ChatGPT-like Conversation</li>
                <li style="margin-bottom: 8px;"> Live Performance Analysis</li>
                <li style="margin-bottom: 8px;"> Dynamic Question Generation</li>
                <li style="margin-bottom: 8px;"> Real-time Scoring & Feedback</li>
                <li style="margin-bottom: 8px;"> Voice & Text Interaction</li>
                <li style="margin-bottom: 8px;"> Context-Aware Responses</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="feature-card" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); border-radius: 20px; padding: 30px; color: white; box-shadow: 0 10px 30px rgba(79, 172, 254, 0.3); transition: transform 0.3s ease, box-shadow 0.3s ease;">
            <div style="font-size: 3rem; margin-bottom: 20px; text-align: center;"> </div>
            <h3 style="font-size: 1.5rem; margin-bottom: 15px; text-align: center;">Smart Resume Builder</h3>
            <p style="line-height: 1.6; margin-bottom: 20px;">Create professional resumes with our intelligent builder that suggests optimal content, formatting, and templates tailored to your target industry and role.</p>
            <ul style="list-style: none; padding: 0;">
                <li style="margin-bottom: 8px;"> Professional Templates</li>
                <li style="margin-bottom: 8px;"> Content Suggestions</li>
                <li style="margin-bottom: 8px;"> Real-time Preview</li>
                <li style="margin-bottom: 8px;"> Export Options</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="feature-card" style="background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); border-radius: 20px; padding: 30px; color: white; box-shadow: 0 10px 30px rgba(250, 112, 154, 0.3); transition: transform 0.3s ease, box-shadow 0.3s ease;">
            <div style="font-size: 3rem; margin-bottom: 20px; text-align: center;"> </div>
            <h3 style="font-size: 1.5rem; margin-bottom: 15px; text-align: center;">Learning Roadmap</h3>
            <p style="line-height: 1.6; margin-bottom: 20px;">Get personalized skill development plans based on your career goals and current skill gaps, with recommended courses and projects.</p>
            <ul style="list-style: none; padding: 0;">
                <li style="margin-bottom: 8px;"> Skill Gap Analysis</li>
                <li style="margin-bottom: 8px;"> Learning Paths</li>
                <li style="margin-bottom: 8px;"> Course Recommendations</li>
                <li style="margin-bottom: 8px;"> Progress Tracking</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="feature-card" style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); border-radius: 20px; padding: 30px; color: #333; box-shadow: 0 10px 30px rgba(168, 237, 234, 0.3); transition: transform 0.3s ease, box-shadow 0.3s ease;">
            <div style="font-size: 3rem; margin-bottom: 20px; text-align: center;"> </div>
            <h3 style="font-size: 1.5rem; margin-bottom: 15px; text-align: center;">Job Search Assistant</h3>
            <p style="line-height: 1.6; margin-bottom: 20px;">Find opportunities that match your skills and preferences with our intelligent job search assistant that aggregates listings from multiple platforms.</p>
            <ul style="list-style: none; padding: 0;">
                <li style="margin-bottom: 8px;"> Job Aggregation</li>
                <li style="margin-bottom: 8px;"> Skill Matching</li>
                <li style="margin-bottom: 8px;"> Application Tracking</li>
                <li style="margin-bottom: 8px;"> Salary Insights</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="feature-card" style="background: linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%); border-radius: 20px; padding: 30px; color: #333; box-shadow: 0 10px 30px rgba(252, 182, 159, 0.3); transition: transform 0.3s ease, box-shadow 0.3s ease;">
            <div style="font-size: 3rem; margin-bottom: 20px; text-align: center;"> </div>
            <h3 style="font-size: 1.5rem; margin-bottom: 15px; text-align: center;">Analytics Dashboard</h3>
            <p style="line-height: 1.6; margin-bottom: 20px;">Track your progress, monitor application success rates, and get insights into your career development with comprehensive analytics.</p>
            <ul style="list-style: none; padding: 0;">
                <li style="margin-bottom: 8px;"> Progress Tracking</li>
                <li style="margin-bottom: 8px;"> Success Metrics</li>
                <li style="margin-bottom: 8px;"> Trend Analysis</li>
                <li style="margin-bottom: 8px;"> Performance Reports</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
"""
    ...
    def main(self):
        ...
        with st.sidebar:
            ...
            for page_name in self.pages.keys():
                if st.button(page_name, use_container_width=True):
                    cleaned_name = page_name.lower().replace(" ", "_").replace("🏠", "").replace("🔍", "").replace("📝", "").replace("📊", "").replace("🎯", "").replace("🎤", "").replace("🎙️", "").replace("🗺️", "").replace("💬", "").replace("ℹ️", "").strip()
                    st.session_state.page = cleaned_name
                    st.rerun()
        ...
        page_mapping = {name.lower().replace(" ", "_").replace("🏠", "").replace("🔍", "").replace("📝", "").replace("📊", "").replace("🎯", "").replace("🎤", "").replace("🎙️", "").replace("🗺️", "").replace("💬", "").replace("ℹ️", "").strip(): name 
    ...
    def render_interview_coach(self):
        """Render the interview coach page"""
        render_interview_coach_interface()

    def render_voice_mock_interview(self):
        """Voice Mock Interview"""
        render_voice_mock_interview_page()

    def render_resume_based_interview_page(self):
        """Render Resume-Based Interview page"""
        render_resume_based_interview()

    def render_learning_roadmap(self):
        """Render the learning roadmap page"""
        st.title("🗺️ Skill Learning Roadmap")
        st.write("Get a personalized learning plan based on your skills and career goals")
        
        # Check if resume analysis is available
        if 'resume_analysis' not in st.session_state:
            st.warning("Please analyze your resume first to generate a personalized roadmap.")
            st.info("Go to the Resume Analyzer page to upload and analyze your resume.")
            return
        ...
        with col1:
            target_role = st.selectbox(
                "Target Career Role",
                ["Software Engineer", "Web Developer", "Data Analyst", "DevOps Engineer", "Product Manager"],
        ...
        if st.button("Generate Learning Roadmap", type="primary"):
            with st.spinner("Generating your personalized learning roadmap..."):
                roadmap_data = self.roadmap_generator.generate_roadmap(
                    current_skills, target_role, time_commitment, learning_style
                )
        ...
        if 'learning_roadmap' in st.session_state:
            ...
            with col2:
                st.metric("Estimated Duration", roadmap['estimated_completion'])
            
            with col3:
                st.metric("Skills Gap", f"{len(roadmap['skill_gap_analysis']['missing_skills'])} skills")
            
            with col4:
                match_percentage = roadmap['skill_gap_analysis']['match_percentage']
                st.metric("Current Match", f"{match_percentage:.1f}%")
            ...
            with col1:
                ...
                matched_skills = roadmap['skill_gap_analysis']['matched_skills']
                if matched_skills:
            ...
            with col2:
                ...
                missing_skills = roadmap['skill_gap_analysis']['missing_skills']
                if missing_skills:
                    for skill in missing_skills[:10]:
                        st.write(f"🎯 {skill}")
                else:
                    st.write("All required skills acquired!")
            
            # Learning phases
            st.subheader("📚 Learning Phases")
            
            for phase in roadmap['learning_phases']:
                with st.expander(f"{phase['title']} ({phase['duration_weeks']} weeks)", expanded=False):
                    st.write(f"**Skills:** {', '.join([s['skill'] for s in phase['skills']])}")
            ...
            insights = roadmap['career_insights']
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.write("**Market Information:**")
                st.write(f"• Job Market Demand: {insights['job_market_demand']}")
                st.write(f"• Salary Range: {insights['average_salary_range']}")
                st.write(f"• Growth Potential: {insights['growth_potential']}")
            
            with col2:
                st.write("**Industry Trends:**")
                for trend in insights['industry_trends']:
                    st.write(f"🔥 {trend}")
            ...
            with col1:
                if st.button("Export as Text"):
                    roadmap_text = self.roadmap_generator.export_roadmap(roadmap)
                    st.download_button(
            ...
            with col2:
                if st.button("Save to Session"):
                    st.success("Roadmap saved to session! You can access it anytime.")
            
            # Generate new roadmap button
            if st.button("Generate New Roadmap"):
                del st.session_state.learning_roadmap
                st.rerun()
